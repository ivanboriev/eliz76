<template>
    <div>
        <v-toolbar>
            ...
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="formAddCluster = true">Добавить кластер</v-btn>
        </v-toolbar>
        <draggable v-model="clusters" handle=".sortHandle" @end="dragReorder">
            <transition-group>
                <v-card class="mt-1" v-for="cluster in clusters" :key="cluster.id">
                    <v-card-title>
                        ...
                        <v-col xs="12" sm="4">
                            <v-text-field
                                ...
                                @click:append="editAndUpdateClusterName(cluster.id, cluster.name)"
                                :append-icon="clustersReadOnly ? 'mdi-pencil' : 'mdi-content-save'"
                                placeholder="Название кластера"
                                @keyup.enter="editAndUpdateClusterName(cluster.id, cluster.name)"
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-btn v-on="on" color="red" @click="openFormDeleteCluster(cluster)" icon>
        ...
        <v-dialog width="400" v-model="formAddCluster">
            <v-card>
                ...
                <v-card-text>
                    <v-text-field autofocus label="Название" v-model="clusterName"></v-text-field>
                </v-card-text>
                <v-card-actions>
        ...
        <v-dialog width="400" v-model="formDeleteCluster">
            <v-card>
                ...
                <v-card-text>Вы действитель хотите удалить кластер {{clusterObj.name}} ?</v-card-text>
                <v-card-actions>
                    <v-btn @click="closeFormDeleteCluster">Нет</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn color="red" @click="deleteCluster(clusterObj.id)">Да</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
